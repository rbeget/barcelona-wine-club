...
export default function App() {
  ...
  const [ratings, setRatings] = useState<Rating[]>([
    {
      ...
      overall: 4.7,
      notes:
        'Complex nose with dark fruit, mineral notes, and hints of Mediterranean herbs. Rich and elegant palate.',
      date: '2025-11-20',
    },
  ]);
  ...
  const handleAddWineClick = () => setCurrentView('addWine');
  ...
  const selectedWine = wines.find(
    (w) => w.id === selectedWineId
  );

  const selectedWineRating = ratings.find(
    (r) => r.wineId === selectedWineId
  );

  const selectedWineEvent = selectedWine
    ? events.find((e) => e.id === selectedWine.eventId)
    : undefined;
